...
A Laravel backend that maintains an append-only ledger of financial movements for investment clients: deposits, withdrawals, and buying/selling instruments. Cash balance is maintained transactionally on the client for fast, concurrency-safe balance checks, while portfolio holdings are always derived from the transaction ledger.
...
**Optional query filters:** `?type=buy` (deposit/withdrawal/buy/sell) and `?instrument=AAPL` — combinable, e.g. `?type=sell&instrument=AAPL`.
...
For more request/response examples covering validation and business-rule edge cases, see the automated test suite in `tests/Feature/`.
...
- **Append-only ledger** — the API only allows new transactions to be appended; existing transactions cannot be updated or deleted through the API.
...
I keep the cash balance as a stored column on the client, rather than deriving it from the ledger on every read. This avoids recalculating the full cash ledger for every transaction, and gives concurrent operations a single row to serialize against — the database transaction then keeps the balance update and the ledger entry atomic. Holdings, on the other hand, are always derived from the ledger, because they're multi-dimensional (a new instrument per client at any time) and a separate table for them would add synchronization risk without a real benefit at this scale.

I chose pessimistic locking (`lockForUpdate()`) over optimistic locking (a version column + retry-on-conflict) for concurrency control. Optimistic locking works well when write conflicts are rare, but for financial writes I'd rather a second request wait briefly than fail and require a retry — correctness and simplicity win over throughput at this scale, and the number of concurrent writes per single client account is realistically very low.
...
One subtlety worth noting: `bcmul()` truncates rather than rounds at the given scale (e.g. `3 × 33.333 = 99.999` becomes `99.99`). The specification does not define a rounding policy, so I kept the behaviour deterministic rather than introducing an arbitrary rounding rule. In a production system, the required monetary rounding policy should be clarified with the business and implemented explicitly.
